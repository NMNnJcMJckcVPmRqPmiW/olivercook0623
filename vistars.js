...
viStars.prototype.draw = function(cxt) {
  cxt.save();
  var xc = 40 * this.s / 4;
...
viStars.prototype.update = function() { 
...
viStarsList = function() {
  this.list = [];
}
viStarsList.prototype.push = function(viStars) {
  this.list.push(viStars);
}
viStarsList.prototype.update = function() {
  for(var i = 0, len = this.list.length; i < len; i++) {
    this.list[i].update();
  }
}
viStarsList.prototype.draw = function(cxt) {
  for(var i = 0, len = this.list.length; i < len; i++) {
    this.list[i].draw(cxt);
  }
}
viStarsList.prototype.get = function(i) {
  return this.list[i];
}
viStarsList.prototype.size = function() {
...
function startviStars() {
  ...
  canvas.setAttribute('id', 'canvas_viStars');
  document.getElementsByTagName('body')[0].appendChild(canvas);
  cxt = canvas.getContext('2d');
  var viStarsList = new viStarsList();
  for(var i = 0; i < 20; i++) {
    var viStars, randomX, randomY, randomS, randomR, randomFnx, randomFny;
    randomX = getRandom('x');
    randomY = getRandom('y');
    randomR = getRandom('r');
    randomS = getRandom('s');
    ...
    viStars = new viStars(randomX, randomY, randomS, randomR, {
      x: randomFnx,
      y: randomFny,
      r: randomFnR
    });
    viStars.draw(cxt);
    viStarsList.push(viStars);
  }
  stop = requestAnimationFrame(function() {
    cxt.clearRect(0, 0, canvas.width, canvas.height);
    viStarsList.update();
    viStarsList.draw(cxt);
...
function stopp() {
  if(staticx) {
    var child = document.getElementById("canvas_viStars");
